window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      add: 'Add server', importSsh: 'Import from SSH config', importSelected: 'Import selected', importEmpty: 'No hosts found in SSH config', save: 'Save server', delete: 'Delete', test: 'Test connection',
      refresh: 'Refresh', terminal: 'Terminal', terminalClose: 'Close', terminalFull: 'Full screen', terminalWindow: 'Window', active: 'Active', makeActive: 'Make active', connected: 'Online', offline: 'Offline', latency: 'Latency', lastError: 'Last error', monthTraffic: 'Month traffic', partial: 'Partial',
      noServers: 'No servers configured yet.', noSnapshot: 'No snapshot available.',
      name: 'Name', host: 'Host', proxyJump: 'Jump host', port: 'Port', username: 'Username', auth: 'Authentication', shellLabel: 'Remote shell', shellPosix: 'POSIX (sh)', shellPowerShell: 'PowerShell',
      ...
      system: 'System', resources: 'Resources', disks: 'Disks', diskPressure: 'A filesystem is over 85% full', processes: 'Top processes', containers: 'Containers',
      network: 'Network', ports: 'Listening ports', hostname: 'Hostname', os: 'OS', kernel: 'Kernel',
      architecture: 'Architecture', cpu: 'CPU', memoryShort: 'Memory', diskShort: 'Disk', noHistory: 'No history yet', uptime: 'Uptime', load: 'Load average', memory: 'Memory', swap: 'Swap',
      used: 'used', available: 'available', filesystem: 'Filesystem', mount: 'Mount', command: 'Command',
      state: 'State', image: 'Image', interface: 'Interface', networkInterface: 'Primary interface', tags: 'Tags', search: 'Search', allTags: 'All', rx: 'RX', tx: 'TX', protocol: 'Protocol',
    ...
    const zh = {
      ...
      add: '添加服务器', importSsh: '从 SSH 配置导入', importSelected: '导入所选', importEmpty: 'SSH 配置中没有主机', save: '保存服务器', delete: '删除', test: '测试连接',
      refresh: '刷新', terminal: '终端', terminalClose: '关闭', terminalFull: '全屏', terminalWindow: '窗口', active: '当前', makeActive: '设为当前', connected: '在线', offline: '离线', latency: '延迟', lastError: '最近错误', monthTraffic: '本月流量', partial: '部分可用',
      noServers: '尚未配置服务器。', noSnapshot: '暂无状态快照。',
      name: '名称', host: '主机', proxyJump: '跳板机', port: '端口', username: '用户名', auth: '认证方式', shellLabel: '远程 Shell', shellPosix: 'POSIX (sh)', shellPowerShell: 'PowerShell',
      key: '私钥', password: '密码', passphrase: '密钥口令', keyPath: '密钥路径',
      keyHint: '密钥只存储在此插件的保险库中。', keepSecret: '留空以保留已存储的值',
      system: '系统', resources: '资源', disks: '磁盘', diskPressure: '有文件系统使用率超过 85%', processes: '高负载进程', containers: '容器',
      network: '网络', ports: '监听端口', hostname: '主机名', os: '操作系统', kernel: '内核',
      architecture: '架构', cpu: 'CPU', memoryShort: '内存', diskShort: '磁盘', noHistory: '暂无历史', uptime: '运行时间', load: '平均负载', memory: '内存', swap: '交换分区',
      used: '已用', available: '可用', filesystem: '文件系统', mount: '挂载点', command: '命令',
      state: '状态', image: '镜像', interface: '接口', networkInterface: '主网卡', tags: '标签', search: '搜索', allTags: '全部', rx: '接收', tx: '发送', protocol: '协议',
    ...
    function formatBytes(value) { const n = Number(value) || 0; if (n >= 1073741824) return `${(n / 1073741824).toFixed(1)} GB`; if (n >= 1048576) return `${(n / 1048576).toFixed(1)} MB`; return `${Math.round(n)} B`; }
    function monthTraffic(profile, t) { return `${t.monthTraffic} ↓ ${formatBytes(profile?.monthRx)} / ↑ ${formatBytes(profile?.monthTx)}`; }
...
.dsm-trends{display:flex;gap:8px;flex-wrap:wrap}.dsm-trend{min-width:160px;flex:1}.dsm-spark{position:relative}.dsm-spark-svg{width:100%;height:36px;display:block}.dsm-spark-tip{position:absolute;right:0;top:0;font-size:10px;color:var(--dsw-alias-label-primary);background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);border-radius:6px;padding:2px 6px}
.dsm-drawer-host{position:fixed;right:12px;bottom:12px;z-index:20}
.dsm-drawer-panel{position:absolute;right:0;bottom:40px;width:min(420px,calc(100vw - 24px));max-height:70vh;overflow:auto;background:var(--dsw-alias-bg-layer-2);border:1px solid var(--dsw-alias-border-l2);border-radius:10px;padding:8px}
`;
    ...
    function useMonitor(ctx) {
      ...
      const refresh = React.useCallback(async (profileId, force = false) => {
        try {
          const targetId = profileId || activeIdRef.current;
          let profiles = state.profiles;
          let id = targetId;
          if (!profiles.length || !id) {
            const base = await request('/state');
            profiles = base.profiles || [];
            id = targetId || base.activeId || profiles[0]?.id || '';
          }
          activeIdRef.current = id;
          const snapshot = id ? (await request(`/snapshot?profileId=${encodeURIComponent(id)}${force ? '&force=1' : ''}`)).snapshot : null;
          setState({ profiles, activeId: id, snapshot, loading: false, error: '' });
      ...
      React.useEffect(() => {
        ...
        const tick = () => {
          if (!doc || !doc.hidden) {
            ...
            if (currentId) {
              request(`/snapshot?profileId=${encodeURIComponent(currentId)}`)
                .then((res) => {
                  if (activeIdRef.current === currentId) {
                    setState((old) => ({ ...old, snapshot: res.snapshot, error: '' }));
                  }
                })
                .catch((error) => {
        ...
        const onVisibility = () => { if (!doc?.hidden) tick(); };
        doc?.addEventListener?.('visibilitychange', onVisibility);
        return () => { clearInterval(timer); doc?.removeEventListener?.('visibilitychange', onVisibility); };
      }, [refresh]);
      const selectActive = React.useCallback(async (profileId) => {
        try {
          await request('/profiles/active', { method: 'POST', body: JSON.stringify({ id: profileId }) });
          await refresh(profileId, true);
        } catch (error) {
          setState((old) => ({ ...old, loading: false, error: error.message }));
        }
      }, [refresh]);
      return { state, refresh, selectActive };
    }
    ...
    function SnapshotView({ snapshot, t }) {
      ...
      return React.createElement(React.Fragment, null,
        ...
        React.createElement(ListSection, { title: t.disks, rows: snapshot.disks, warnAt: 85, warnText: t.diskPressure, columns: [t.filesystem, t.mount, t.used], render: (row) => [row.filesystem, row.mount, `${formatMb(row.usedMb)} / ${formatMb(row.totalMb)} (${pct(row.percent)})`] }),
        ...
        React.createElement(ListSection, { title: t.network, rows: snapshot.network?.interfaces, columns: [t.interface, t.rx, t.tx], render: (row) => [row.name, `${row.rxBytes} B · ${row.rxBytesPerSec || 0} B/s`, `${row.txBytes} B · ${row.txBytesPerSec || 0} B/s`] }),
    ...
    function TerminalModal({ profile, t, onClose }) {
      ...
      React.useEffect(() => {
        ...
        let observer;
        let closed = false;
        const styleId = 'dsm-term-css';
        if (!document.getElementById(styleId)) {
          const style = document.createElement('style');
          style.id = styleId;
          style.textContent = '.dsm-term{position:fixed;inset:16px;z-index:50;display:flex;flex-direction:column;background:var(--dsw-alias-bg-layer-2);color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l1);border-radius:12px;overflow:hidden}.dsm-term-full{inset:0;border-radius:0}.dsm-term-bar{display:flex;justify-content:space-between;gap:8px;padding:8px 12px}.dsm-term-host{flex:1;min-height:240px;padding:8px}';
          document.head.appendChild(style);
        }
        if (!document.getElementById('dsm-xterm-css')) {
          const link = document.createElement('link');
          link.id = 'dsm-xterm-css';
          link.rel = 'stylesheet';
          link.href = API + '/vendor/xterm.css';
          document.head.appendChild(link);
        }
        (async () => {
          try {
            const [{ Terminal }, { FitAddon }] = await Promise.all([
              import(API + '/vendor/xterm.mjs'),
              import(API + '/vendor/addon-fit.mjs')
            ]);
            if (closed || !host.current) return;
            term = new Terminal({ scrollback: 5000, cursorBlink: true, fontFamily: 'ui-monospace, monospace' });
            const fit = new FitAddon();
            term.loadAddon(fit);
            term.open(host.current);
            fit.fit();
            const protocol = location.protocol === 'https:' ? 'wss:' : 'ws:';
            socket = new WebSocket(protocol + '//' + location.host + API + '/pty?profileId=' + encodeURIComponent(profile.id));
            socket.binaryType = 'arraybuffer';
            socket.onmessage = (event) => term.write(typeof event.data === 'string' ? event.data : new Uint8Array(event.data));
            term.onData((data) => { if (socket.readyState === 1) socket.send(data); });
            const sendSize = () => {
              fit.fit();
            ...
            if (!closed) setFailed(error instanceof Error ? error.message : String(error));
          }
        })();
        return () => {
          closed = true;
      ...
      return React.createElement('div', { className: full ? 'dsm-term dsm-term-full' : 'dsm-term' },
        React.createElement('div', { className: 'dsm-term-bar' },
          React.createElement('strong', null, profile.name),
          React.createElement('div', null,
            React.createElement('button', { className: 'dsm-btn', type: 'button', onClick: () => setFull((value) => !value) }, full ? t.terminalWindow : t.terminalFull),
            React.createElement('button', { className: 'dsm-btn', type: 'button', onClick: onClose }, t.terminalClose)
          )
        ),
        failed ? React.createElement('div', { className: 'dsm-error' }, failed) : null,
        React.createElement('div', { className: 'dsm-term-host', ref: host })
      );
    }
    function MonitorPane({ ctx }) {
      ensureCss();
      const t = labels(ctx);
      const { state, refresh, selectActive } = useMonitor(ctx);
      const [query, setQuery] = React.useState('');
      const [tag, setTag] = React.useState('');
      const [importList, setImportList] = React.useState(null);
      ...
      const header = React.createElement('div', { className: 'dsm-card' },
        React.createElement('div', { className: 'dsm-head' },
          React.createElement('div', null,
            ...
            React.createElement('div', { className: 'dsm-sub' }, profile ? `${profile.name} · ${profile.username}@${profile.host}${profile.latencyMs == null ? '' : ` · ${t.latency} ${profile.latencyMs} ms`}` : t.noServers), profile?.lastError ? React.createElement('div', { className: 'dsm-error' }, `${t.lastError}: ${profile.lastError}`) : null, profile ? React.createElement('div', { className: 'dsm-muted' }, monthTraffic(profile, t)) : null, profile ? React.createElement(Trends, { profile, t }) : null
          ),
          React.createElement('div', null,
            profile ? React.createElement('button', { className: 'dsm-btn', type: 'button', onClick: () => setTermOpen(true) }, t.terminal) : null,
            React.createElement('button', { className: 'dsm-btn', type: 'button', onClick: () => refresh(id, true) }, t.refresh)
          )
        ),
        state.profiles.length ? React.createElement(React.Fragment, null, React.createElement(HostFilter, { profiles: state.profiles, query, tag, onQuery: setQuery, onTag: setTag, t }), React.createElement('select', {
          className: 'dsm-input', value: id,
          onChange: (event) => selectActive(event.target.value)
        }, state.profiles.filter((item) => item.id === id || matchesProfile(item, query, tag)).map((item) => React.createElement('option', { key: item.id, value: item.id }, `${item.name} (${item.username}@${item.host})`)))) : null
      );
      ...
      const modal = termOpen && profile ? React.createElement(TerminalModal, { profile, t, onClose: () => setTermOpen(false) }) : null;
      return React.createElement('div', { className: 'dsm-root' }, header, body, modal);
    }
    ...
    function profileTags(profile) { return Array.isArray(profile?.tags) ? profile.tags : String(profile?.tags || '').split(',').map((item) => item.trim()).filter(Boolean); }
    function matchesProfile(profile, query, tag) {
      const haystack = `${profile?.name || ''} ${profile?.username || ''} ${profile?.host || ''} ${profileTags(profile).join(' ')}`.toLowerCase();
      if (query && !haystack.includes(String(query).trim().toLowerCase())) return false;
      return !tag || profileTags(profile).includes(tag);
    }
    function HostFilter({ profiles, query, tag, onQuery, onTag, t }) {
      const tags = [...new Set(profiles.flatMap(profileTags))];
      return React.createElement('div', { className: 'dsm-row' },
        React.createElement('input', { className: 'dsm-input', placeholder: t.search, value: query, onChange: (event) => onQuery(event.target.value) }),
        React.createElement('button', { className: 'dsm-btn', type: 'button', disabled: !tag, onClick: () => onTag('') }, t.allTags),
        ...tags.map((item) => React.createElement('button', { className: 'dsm-btn', type: 'button', key: item, disabled: tag === item, onClick: () => onTag(item) }, item))
      );
    }

    const historyCache = new Map();
    function loadSeries(profileId, field) {
      const key = `${profileId}:${field}`;
      const cached = historyCache.get(key);
      if (cached && Date.now() - cached.at < 30000) return Promise.resolve(cached.points);
      const from = Date.now() - 60 * 60 * 1000;
      return request(`/history?profileId=${encodeURIComponent(profileId)}&field=${field}&from=${from}&to=${Date.now()}`).then((body) => {
    ...
    function sparkColor(value) {
      if (value >= 85) return 'var(--dsw-alias-state-error-primary)';
      if (value >= 60) return 'var(--dsw-alias-state-warning-primary)';
    ...
    function Sparkline({ points = [] }) {
      ...
      const width = 160;
      const height = 36;
      if (!points.length) return React.createElement('div', { className: 'dsm-muted' }, '—');
      const minT = points[0].t;
      const span = Math.max(1, points[points.length - 1].t - minT);
      let path = '';
      points.forEach((point, index) => {
        const x = ((point.t - minT) / span) * width;
        const y = height - (Math.max(0, Math.min(100, Number(point.value) || 0)) / 100) * height;
        const gap = index > 0 && point.t - points[index - 1].t > 90000;
        path += `${index === 0 || gap ? 'M' : 'L'}${x.toFixed(1)},${y.toFixed(1)}`;
      });
      const last = Number(points[points.length - 1].value) || 0;
      const onMove = (event) => {
        const rect = event.currentTarget.getBoundingClientRect();
        const ratio = rect.width ? (event.clientX - rect.left) / rect.width : 0;
        const target = minT + ratio * span;
        const nearest = points.reduce((best, point) => Math.abs(point.t - target) < Math.abs(best.t - target) ? point : best, points[0]);
        setHover({ ...nearest, x: ((nearest.t - minT) / span) * width });
      };
      return React.createElement('div', { className: 'dsm-spark' },
        React.createElement('svg', { viewBox: `0 0 ${width} ${height}`, preserveAspectRatio: 'none', className: 'dsm-spark-svg', onMouseMove: onMove, onMouseLeave: () => setHover(null) },
          [0, 50, 100].map((level) => React.createElement('line', { key: level, x1: 0, x2: width, y1: height - (level / 100) * height, y2: height - (level / 100) * height, stroke: 'var(--dsw-alias-border-l2)', strokeDasharray: '2 3' })),
          React.createElement('path', { d: path, fill: 'none', stroke: sparkColor(last), strokeWidth: 1.5 }),
          hover ? React.createElement('line', { x1: hover.x, x2: hover.x, y1: 0, y2: height, stroke: 'var(--dsw-alias-label-secondary)' }) : null
        ),
        hover ? React.createElement('div', { className: 'dsm-spark-tip' }, `${new Date(hover.t).toLocaleTimeString()} · ${hover.value}`) : null
      );
    }
    function Trend({ profileId, field, label, empty }) {
      const [points, setPoints] = React.useState(null);
      React.useEffect(() => {
        let alive = true;
        loadSeries(profileId, field).then((next) => { if (alive) setPoints(next); }).catch(() => { if (alive) setPoints([]); });
        return () => { alive = false; };
      }, [profileId, field]);
      return React.createElement('div', { className: 'dsm-trend' },
        React.createElement('div', { className: 'dsm-label' }, label),
        points === null ? React.createElement('div', { className: 'dsm-muted' }, '…') : points.length ? React.createElement(Sparkline, { points }) : React.createElement('div', { className: 'dsm-muted' }, empty)
      );
    }
    function Trends({ profile, t }) {
      if (!profile?.id) return null;
      return React.createElement('div', { className: 'dsm-trends' },
        React.createElement(Trend, { profileId: profile.id, field: 'cpu', label: t.cpu, empty: t.noHistory }),
        React.createElement(Trend, { profileId: profile.id, field: 'ram', label: t.memoryShort, empty: t.noHistory }),
        React.createElement(Trend, { profileId: profile.id, field: 'disk', label: t.diskShort, empty: t.noHistory })
      );
    }
    function SettingsCard({ ctx }) {
      ensureCss();
      const t = labels(ctx);
      const [state, setState] = React.useState({ profiles: [], activeId: '', loading: true });
      const [importList, setImportList] = React.useState(null);
      const [picked, setPicked] = React.useState({});
      ...
      const load = React.useCallback(() => request('/state').then((next) => setState({ profiles: next.profiles || [], activeId: next.activeId || '', loading: false })).catch((e) => { setError(e.message); setState((old) => ({ ...old, loading: false })); }), []);
      ...
      const field = (key, label, type = 'text') => React.createElement('div', { className: 'dsm-field' },
        ...
        React.createElement('input', { className: 'dsm-input', type, value: key === 'tags' ? (Array.isArray(draft?.[key]) ? draft[key].join(', ') : (draft?.[key] ?? '')) : (draft?.[key] ?? ''), onChange: (event) => setDraft((old) => ({ ...old, [key]: type === 'number' ? Number(event.target.value) : event.target.value })) })
      );
      const save = async () => {
      ...
      const visibleProfiles = state.profiles.filter((profile) => matchesProfile(profile, query, tag));
      const profileRows = visibleProfiles.map((profile) => React.createElement('div', { className: 'dsm-row', key: profile.id },
        ...
        profile.latencyMs == null ? null : React.createElement('span', { className: 'dsm-muted' }, `${t.latency} ${profile.latencyMs} ms`),
        profile.lastError ? React.createElement('span', { className: 'dsm-error' }, profile.lastError) : null,
        React.createElement('span', { className: 'dsm-muted' }, monthTraffic(profile, t)), React.createElement(Trends, { profile, t }),
        React.createElement('button', {
          className: 'dsm-btn',
          type: 'button',
          disabled: state.activeId === profile.id,
          onClick: async () => {
            try {
              await request('/profiles/active', { method: 'POST', body: JSON.stringify({ id: profile.id }) });
              await load();
            } catch (e) { setError(e.message); }
          }
        }, state.activeId === profile.id ? t.active : t.makeActive),
      ...
      const form = draft ? React.createElement('div', { className: 'dsm-card' },
        ...
        React.createElement('div', { className: 'dsm-form-grid' },
          field('name', t.name), field('host', t.host), field('proxyJump', t.proxyJump), field('port', t.port, 'number'), field('username', t.username),
          field('networkInterface', t.networkInterface), field('tags', t.tags), field('privateKeyPath', t.keyPath), field('privateKey', t.key, 'password'), field('passphrase', t.passphrase, 'password'), field('password', t.password, 'password'), React.createElement('div', { className: 'dsm-field' }, React.createElement('label', null, t.auth), React.createElement('select', { className: 'dsm-input', value: draft?.authType || 'key', onChange: (event) => setDraft((old) => ({ ...old, authType: event.target.value })) }, React.createElement('option', { value: 'key' }, t.key), React.createElement('option', { value: 'password' }, t.password))), React.createElement('div', { className: 'dsm-field' }, React.createElement('label', null, t.shellLabel), React.createElement('select', { className: 'dsm-input', value: draft?.shell === 'powershell' ? 'powershell' : 'posix', onChange: (event) => setDraft((old) => ({ ...old, shell: event.target.value })) }, React.createElement('option', { value: 'posix' }, t.shellPosix), React.createElement('option', { value: 'powershell' }, t.shellPowerShell)))
        ),
        keygenBlock,
      ...
      return React.createElement('div', { className: 'dsm-root' },
        React.createElement('div', { className: 'dsm-card' },
          React.createElement('div', { className: 'dsm-head' },
            ...
            React.createElement('div', { className: 'dsm-row' },
              React.createElement('button', { className: 'dsm-btn', type: 'button', onClick: async () => { const body = await request('/profiles/import-ssh-config'); const candidates = body.candidates || []; setImportList(candidates); setPicked(Object.fromEntries(candidates.map((item) => [item.name, true]))); } }, t.importSsh),
              React.createElement('button', { className: 'dsm-btn dsm-btn-primary', type: 'button', onClick: () => edit(null) }, t.add)
            )
          ),
          state.loading ? React.createElement('div', { className: 'dsm-muted' }, t.loading) : state.profiles.length ? React.createElement(React.Fragment, null, React.createElement(HostFilter, { profiles: state.profiles, query, tag, onQuery: setQuery, onTag: setTag, t }), profileRows.length ? profileRows : React.createElement('div', { className: 'dsm-muted' }, t.noServers)) : error ? null : React.createElement('div', { className: 'dsm-muted' }, t.noServers)
        ),
        updaterBlock,
        noticeBlock,
        importList ? React.createElement('div', { className: 'dsm-card' },
          React.createElement('div', { className: 'dsm-section-title' }, t.importSsh),
          importList.length ? importList.map((item) => React.createElement('label', { key: item.name, className: 'dsm-row' },
            React.createElement('input', { type: 'checkbox', checked: Boolean(picked[item.name]), onChange: (event) => setPicked((old) => ({ ...old, [item.name]: event.target.checked })) }),
            React.createElement('span', null, `${item.name} · ${item.username || 'root'}@${item.host}:${item.port}`)
          )) : React.createElement('div', { className: 'dsm-muted' }, t.importEmpty),
          React.createElement('div', { className: 'dsm-row' },
            React.createElement('button', { className: 'dsm-btn dsm-btn-primary', type: 'button', onClick: async () => {
              for (const item of importList) {
                if (!picked[item.name]) continue;
                const id = item.name.replace(/[^A-Za-z0-9_-]+/g, '-').replace(/^-|-$/g, '').slice(0, 64) || 'host';
                await request('/profiles/save', { method: 'POST', body: JSON.stringify({ id, name: item.name, host: item.host, port: item.port, username: item.username || 'root', privateKeyPath: item.privateKeyPath || '', proxyJump: item.proxyJump || '', authType: 'key' }) });
              }
              setImportList(null);
              await load();
            } }, t.importSelected),
            React.createElement('button', { className: 'dsm-btn', type: 'button', onClick: () => setImportList(null) }, t.cancel)
          )
        ) : null,
        form,
    ...
    function mountDrawer(ctx) {
      ...
      ensureCss();
      let ReactDOM = null;
      try { ReactDOM = require('react-dom/client'); } catch { try { ReactDOM = require('react-dom'); } catch { ReactDOM = null; } }
      if (!ReactDOM) return () => {};
      const host = document.createElement('div');
      host.className = 'dsm-drawer-host';
      host.setAttribute('data-dsh-plugin', NS);
      document.body.appendChild(host);
      function Drawer() {
        const [open, setOpen] = React.useState(false);
        return React.createElement('div', { className: 'dsm-drawer' },
          React.createElement('button', { className: 'dsm-btn dsm-drawer-toggle', type: 'button', onClick: () => setOpen((value) => !value) }, labels(ctx).servers),
          open ? React.createElement('div', { className: 'dsm-drawer-panel' }, React.createElement(MonitorPane, { ctx })) : null
        );
      }
      if (typeof ReactDOM.createRoot === 'function') {
        const root = ReactDOM.createRoot(host);
        root.render(React.createElement(Drawer));
        return () => { root.unmount(); host.remove(); };
      }
      if (typeof ReactDOM.render === 'function') {
        ReactDOM.render(React.createElement(Drawer), host);
        return () => { ReactDOM.unmountComponentAtNode?.(host); host.remove(); };
      }
      host.remove();
      return () => {};
    }
    function register(ctx) {
      ...
      let sidebarPlaced = false; let removeDrawer = null; const markSidebarPlaced = () => { sidebarPlaced = true; if (removeDrawer) { removeDrawer(); removeDrawer = null; } }; try { ctx.inject?.(['sidebarRightTabs'], (sctx) => { const tabs = sctx?.sidebarRightTabs; if (!tabs?.register) return; markSidebarPlaced(); const t = labels(ctx); const definition = { id: '@goodandready/dsh-server-monitor', kind: 'server-monitor', priority: 'extension', title: () => t.servers, guide: [{ order: 40, title: () => t.title, description: () => t.guideDescription, icon: () => React.createElement('span', null, '◫') }] }; const registerTab = () => tabs.register(definition); if (sctx.effect) sctx.effect(registerTab); else registerTab(); const registerPane = () => ctx.slots?.register?.({ name: 'sidebar.right.pane.tab', key: '@goodandready/dsh-server-monitor', locale: NS, inject: () => ({ ctx }) }, MonitorPane); if (ctx.slots?.inject) ctx.slots.inject('sidebar.right.pane.tab', registerPane); else registerPane(); }); } catch (error) { console.warn('[dsh-server-monitor] native sidebar registration failed', error); }
      try { ctx.inject?.(['betterSidebar'], (sctx) => { const service = sctx?.betterSidebar; if (!service?.registerTab) return; markSidebarPlaced(); const registerTab = () => service.registerTab({ id: 'dsh-server-monitor:tab', title: () => labels(ctx).servers, icon: () => React.createElement('span', null, '◫'), order: 40, component: ({ scope }) => React.createElement(MonitorPane, { ctx, scope }) }); if (sctx.effect) sctx.effect(registerTab); else registerTab(); }); } catch (error) { console.warn('[dsh-server-monitor] legacy sidebar registration failed', error); }
      if (typeof setTimeout === 'function') setTimeout(() => { if (!sidebarPlaced) removeDrawer = mountDrawer(ctx); }, 0);
    }
