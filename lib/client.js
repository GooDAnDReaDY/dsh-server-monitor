...
`;
    ...
    function SnapshotView({ snapshot, t }) {
      ...
      return React.createElement(React.Fragment, null,
        ...
        React.createElement(ListSection, { title: t.ports, rows: snapshot.ports, columns: [t.protocol, t.address, 'Port'], render: (row) => [row.protocol, row.address, row.port] })
      );
    }
    ...
    function SettingsCard({ ctx }) {
      ...
      const profileRows = state.profiles.map((profile) => React.createElement('div', { className: 'dsm-row', key: profile.id },
        React.createElement('button', { className: 'dsm-btn', type: 'button', onClick: () => edit(profile) }, profile.name),
    ...
    function register(ctx) {
      try { ctx.locale?.register?.(NS, { en, zh }); } catch (error) { console.warn('[dsh-server-monitor] locale registration failed', error); }
      ...
      try { ctx.inject?.(['sidebarRightTabs'], (sctx) => { const tabs = sctx?.sidebarRightTabs; if (!tabs?.register) return; const definition = { id: '@goodandready/dsh-server-monitor', kind: 'server-monitor', priority: 'extension', title: () => 'Servers', guide: [{ order: 40, title: () => 'Server Monitor', description: () => 'Read-only Linux server status', icon: () => React.createElement('span', null, '◫') }] }; const registerTab = () => tabs.register(definition); if (sctx.effect) sctx.effect(registerTab); else registerTab(); const registerPane = () => ctx.slots?.register?.({ name: 'sidebar.right.pane.tab', key: '@goodandready/dsh-server-monitor', locale: NS, inject: () => ({ ctx }) }, MonitorPane); if (ctx.slots?.inject) ctx.slots.inject('sidebar.right.pane.tab', registerPane); else registerPane(); }); } catch (error) { console.warn('[dsh-server-monitor] native sidebar registration failed', error); }
      try { ctx.inject?.(['betterSidebar'], (sctx) => { const service = sctx?.betterSidebar; if (!service?.registerTab) return; const registerTab = () => service.registerTab({ id: 'dsh-server-monitor:tab', title: () => 'Servers', icon: () => React.createElement('span', null, '◫'), order: 40, component: ({ scope }) => React.createElement(MonitorPane, { ctx, scope }) }); if (sctx.effect) sctx.effect(registerTab); else registerTab(); }); } catch (error) { console.warn('[dsh-server-monitor] legacy sidebar registration failed', error); }
    }
    module.exports.inject = ['slots', 'locale'];
    module.exports.apply = register;
    module.exports._test = { en, zh, register };
