import { removeProfileMetrics } from './profile-metrics.js';
import { readSshConfig } from './ssh-config-parser.js';
...
export function isTrustedRequest(req) {
  ...
  // Browser headers are spoofable. They only refine a loopback client.
  if (!isLoopbackAddress(getClientIp(req))) return false;
...
export function createRouteHandlers(webServer, store, collector, sshService, recorder = null, metricStore = null) {
  const snapshotCache = new Map();
  const snapshotIntervalMs = 15000;

  async function getSnapshot(profile, force = false) {
    const now = Date.now();
    if (!force && recorder?.getFreshSnapshot) {
      const recorded = recorder.getFreshSnapshot(profile.id, now);
    ...
    entry.inFlight = Promise.resolve()
      ...
      .then((snapshot) => { entry.snapshot = snapshot; recorder?.remember?.(profile.id, snapshot); return snapshot; }, (error) => { entry.error = error; throw error; })
  ...
  register('/dsh-server-monitor/state', 'GET', async (req, res) => {
    ...
    const profiles = store.getProfiles().map((profile) => {
      const status = sshService?.getHostStatus?.(profile.id) || {};
      const month = metricStore?.readMonth?.(profile.id) || {};
      return { ...profile, latencyMs: status.latencyMs ?? null, lastError: status.lastError || '', monthRx: Number(month.rx) || 0, monthTx: Number(month.tx) || 0 };
    });
    writeJson(res, 200, { ok: true, profiles, activeId: store.getActiveId() || '' });
  });

  register('/dsh-server-monitor/history', 'GET', async (req, res) => {
    if (!trustedRequest(req)) return writeJson(res, 403, { ok: false, error: 'Forbidden' });
    const id = queryValue(req, 'profileId') || store.getActiveId();
    if (!store.getProfile(id)) return writeJson(res, 404, { ok: false, error: 'Profile not found' });
    const from = Number(queryValue(req, 'from') || 0);
    const to = Number(queryValue(req, 'to') || Date.now());
    const field = queryValue(req, 'field') || 'cpu';
    const history = metricStore?.query?.(id, { from, to, field }) || { resolution: null, points: [] };
    writeJson(res, 200, { ok: true, profileId: id, field, ...history });
  });
  ...
  register('/dsh-server-monitor/profiles/delete', 'POST', async (req, res) => {
    ...
    sshService?.closeConnection?.(body.id);
    sshService?.clearHostStatus?.(body.id);
    removeProfileMetrics(body.id);
    writeJson(res, 200, { ok: true });
  });

  register('/dsh-server-monitor/profiles/import-ssh-config', 'GET', async (req, res) => {
    if (!trustedRequest(req)) return writeJson(res, 403, { ok: false, error: 'Forbidden' });
    writeJson(res, 200, { ok: true, candidates: readSshConfig() });
  });
